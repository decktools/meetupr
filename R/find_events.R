...
#' @param n_offsets Numeric. Number of offsets to ask for from the API.
...
#' }
find_events <- function(query = NULL, n_offsets = 10, topic_category = NULL, lat = NULL, lon = NULL, radius = NULL, start_date_range = NULL, end_date_range = NULL, fields = NULL, excluded_groups = NULL, order = NULL, api_key = NULL) {
  ...
  res <- .fetch_events(api_method, api_key, n_offsets = n_offsets, text = query, topic_category = topic_category, lat = lat, lon = lon, radius = radius, start_date_range = start_date_range, end_date_range = end_date_range, fields = fields, excluded_groups = excluded_groups, order = order)
  ...
  res %>%
    ...
    dplyr::mutate(
      dplyr::across(
        c(event_created, event_time),
        .date_helper
      ),
      event_duration = event_duration / 3600000, # Milliseconds -> hours
      resource = list(res)
    )
}
